#!/usr/bin/env python3
"""
PSGC Output vs FHIR CodeSystem Validation Script (Optimized for Large Files)
...
Handles large files more efficiently by using streaming JSON parsing where possible.
"""
...
def print_human_readable_report(report: Dict[str, Any], output_file: Optional[str] = None):
    ...
    # Show limited output to avoid too much text for large files
    max_display_items = 20  # Limit for large outputs
    
    # Code differences
    output_lines.append("CODE DIFFERENCES:")
    output_lines.append("-" * 20)
    
    fhir_unique = report['code_differences']['codes_only_in_fhir']
    psgc_unique = report['code_differences']['codes_only_in_psgc']
    
    if fhir_unique:
        output_lines.append(f"Codes only in FHIR codesystem ({len(fhir_unique)}):")
        for code in sorted(fhir_unique)[:max_display_items]:
            output_lines.append(f"  - {code}")
        if len(fhir_unique) > max_display_items:
            output_lines.append(f"  ... and {len(fhir_unique) - max_display_items} more")
        output_lines.append("")
    
    if psgc_unique:
        output_lines.append(f"Codes only in PSGC output ({len(psgc_unique)}):")
        for code in sorted(psgc_unique)[:max_display_items]:
            output_lines.append(f"  - {code}")
        if len(psgc_unique) > max_display_items:
            output_lines.append(f"  ... and {len(psgc_unique) - max_display_items} more")
        output_lines.append("")
    ...
    parent_diffs = report['hierarchy_differences']['codes_with_different_parents']
    if parent_diffs:
        output_lines.append(f"Codes with different parent assignments ({len(parent_diffs)}):")
        for code in sorted(parent_diffs)[:max_display_items]:
        ...
        if len(parent_diffs) > max_display_items:
            output_lines.append(f"  ... and {len(parent_diffs) - max_display_items} more")
        output_lines.append("")
    else:
    ...
    level_diffs = report['level_differences']['codes_with_different_levels']
    if level_diffs:
        output_lines.append(f"Codes with different geographic level assignments ({len(level_diffs)}):")
        for code in sorted(level_diffs)[:max_display_items]:
        ...
        if len(level_diffs) > max_display_items:
            output_lines.append(f"  ... and {len(level_diffs) - max_display_items} more")
        output_lines.append("")
    else:
...
def main():
    ...
    try:
        ...
        fhir_codes, fhir_parent_map, fhir_level_map = extract_codes_and_hierarchy_optimized(fhir_data)
        
        print("Extracting codes and hierarchies from PSGC output...")
        psgc_codes, psgc_parent_map, psgc_level_map = extract_codes_and_hierarchy_optimized(psgc_data)
